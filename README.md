## Hi there 👋 I'm Birendra Kumar  

💻 **Cloud & DevOps Engineer | AWS | Docker | Kubernetes | Terraform | CI/CD**  
🌍 Based in Ranchi, Jharkhand | Former Software Engineer Associate at DXC Technology  

---

### 🚀 About Me
- Passionate about **Cloud Computing** and **DevOps automation**  
- Experienced in **Linux system administration**, **AWS infrastructure**, and **CI/CD pipelines**  
- Skilled in **Docker**, **Kubernetes**, **Terraform**, **Jenkins**, **Prometheus**, and **Grafana**  
- Strong focus on **secure, scalable, and automated infrastructure solutions**

---

### 🌱 Currently Learning
- Advanced **AWS services** (EKS, ECS, EventBridge, Secrets Manager)  
- **Infrastructure as Code (IaC)** best practices  
- **FinOps** and multi-cloud orchestration  
- **AI/ML integration** for predictive scaling and anomaly detection  

---

### 🧩 Projects
#### 🔹 [Automated Scalable AWS Infrastructure Deployment](https://github.com/Birendrakum/AWS-Project.git)
Infrastructure as Code using CloudFormation to launch a website with dynamic scaling and secure S3 integration.

#### 🔹 [BookMyShow Clone – End-to-End DevOps Deployment](https://github.com/Birendrakum/Plumbing-service-website.git)
Deployed a scalable web app using AWS EKS, Terraform, Jenkins CI/CD, SonarQube, Trivy, Prometheus, and Grafana.

#### 🔹 [Lambda Automation scripts](https://github.com/Birendrakum/AWS-Automation-lambda-.git)
1. Hourly health report of autoscaling group using lambda and eventbridge
2. AWS IAM Access Key Rotation Automation
3. AWS S3 Malware Scanning and Quarantine Solution

#### 🔹 working
---

### 🛠️ Tech Stack
**Cloud:** AWS (EC2, VPC, S3, IAM, CloudWatch, CloudTrail, EventBridge, AWS KMS, Secret Manager, ECS, EKS, SQS, SNS, lambda, AWS Organization, EBS, EFS)  
**DevOps:** Docker, Kubernetes, Jenkins, Terraform, Ansible, Prometheus, Grafana
**Monitoring:** Prometheus, Grafana  
**Languages:** Python, MySQL  
**OS:** Linux  

---

### 📫 Connect With Me
- 💼 .https://www.linkedin.com/in/birendra-kumar-09580b173.(#)  
- 📧 birendrakum.119@gmail.com  

---

⭐ _“Building automation that scales, secures, and simplifies cloud infrastructure.”_

